...
$Text_Rerun.Summary_Content_Logs = "Gather the Azure Stack log files and save to d:\AzureStackLogs"
...
$Text_Completed.Summary_Content = "Gather the Azure Stack log files and save to d:\AzureStackLogs"
#endregion Text
...
$S_Netbxnda = {
    ...
    If ((Get-PnpDeviceProperty -InputObject $PnPDevice -KeyName DEVPKEY_Device_DriverInfPath).Data -eq "netbxnda.inf") {
        ...
        try {
            Start-Transcript -Path D:\CloudDeployment\Setup\netbxnda.txt -Append
            $filepath = "$env:TEMP\netbxnda.exe"
...
Function F_Initialize {
    ...
    if (test-path "D:\CloudDeployment\Setup\InstallAzureStackPOC.ps1") {
        if(!(test-path "D:\CloudDeployment\ECEngine\EnterpriseCloudEngine.psd1")) {
            ...
            Import-Module "D:\CloudDeployment\ECEngine\EnterpriseCloudEngine.psd1" -Force -Verbose:$false
            ...
            # Deployment in progress or stopped
                ...
                if(!(test-path "D:\CloudDeployment\Rerun\config.xml")) {
                    New-Item D:\CloudDeployment\Rerun -type directory -Force
                    '<config status="rerun" run="0"/>' | Out-File D:\CloudDeployment\Rerun\config.xml
                ...
                # Deployed with deployment UI
                    ...
                    $Status = [XML](Get-Content "D:\CloudDeployment\Rerun\config.xml")
...
Function F_Summary {
    If ($Script:Initialized -eq "CloudBuilder_Install") {
        ...
        $InstallScript += 'cd D:\CloudDeployment\Setup'
        $InstallScript += "`r`n"
...
Function F_Install {
    ...
    "cd D:\CloudDeployment\Setup" |  Add-Content $filepath
...
Function F_Rerun {
    ...
    [int]$Run = ([XML](Get-Content "D:\CloudDeployment\Rerun\config.xml")).config.run
    '<config status="rerun" run="' + ($Run+1) + '"/>' | Out-File D:\CloudDeployment\Rerun\config.xml
    #endregion

    #region Rerun
    Set-Location D:\CloudDeployment\Setup
...
Function F_GetAzureStackLogs {
    ...
    #region Logs
    Set-Location D:\CloudDeployment\AzureStackDiagnostics\Microsoft.AzureStack.Diagnostics.DataCollection
    Import-Module .\Microsoft.AzureStack.Diagnostics.DataCollection.psd1
    Get-AzureStackLogs -OutputPath D:\AzureStackLogs
    #endregion
}
#endregion Functions
